    ...
    <div className="p-4 w-1/4">
      <Tooltip content={info?.name} hidden={!info?.menu}>
        {!isNew ? (
          ...
          <div
            ...
            <Plus />
            <p className="w-0 h-6 group-hover:w-auto interpolate-size transition-all overflow-hidden ">Agregar pedido</p>
          </div>
        )}
      </Tooltip>
      ...
      <Modal open={isOpen} onOpenChange={closeModal} title={isNew ? 'Crear pedido' : 'Editar pedido'}>
        <div className="flex flex-col gap-3">
          <label>
            Nombre
            <TextField.Root radius="large" className={`w-full`} value={formInfo?.name} onChange={(e) => setFormInfo({ ...formInfo, name: e.target.value })} />
          </label>
          ...
          <label className="flex flex-col">
            Menú
            <Select value={formInfo?.menu} onChange={selectMenu} items={menus} valueKey="menu" labelKey="menu" />
          </label>
        ...
        <div className="flex w-full justify-between mt-4">
          ...
          <Button disabled={!formInfo?.name || !formInfo?.type || !formInfo?.menu} onClick={isNew ? addNewReq : acceptEdition}>
            {isNew ? 'Crear' : 'Editar'}
          </Button>
        </div>
      </Modal>
